...
namespace Backpack_Problem2_Dynamic_Programming
{
    //the algorithm assumes that we can use each item MANY TIMES!!!
    class Program
    {
        static void Main(string[] args)
        {
            Item[] items = { new Item(23, 45), new Item(15, 60), new Item(21, 17), new Item(13, 28), new Item(21, 51) };
            Console.WriteLine(Backpack2(items, 50));
            Console.ReadKey();
        }
        /// <summary>
        /// 
        ...
        public static int Backpack2(Item[] items, int load)
        {
            ...
            for (int i = 1; i < DP.GetLength(0); i++)
            {
                ...
                for (int j = 1; j < DP.GetLength(1); j++)
                {
                    if (j - items[k].weight >= 0)
                    {
                        ...
                        DP[i, j] = Math.Max(DP[i, j], DP[i, j - items[k].weight] + items[k].value);//additional condition 
                    }
